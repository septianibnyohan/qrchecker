...
function prepareData(save){
  ...
  $('.signer-assembler action').each(function (index, value) {
    var allow_index = $('.signer-assembler action').length - 1;
    if (index == allow_index) {
      var type = $(this).attr('type');

      //20200915, Septian, BPP-37, start
      if (type != 'drawing') {
        actions.push({
          type: $(this).attr('type'),
          page: $(this).attr('page'),
...
function emptyBuilder(){
  ...
  $('.signer-element[resizeable="true"]').draggable("disable");
  $('.signer-element[resizeable="true"]').resizable('destroy');
  $('.signer-element[resizeable="true"] img').attr('src', '/assets/images/signhere_grey.png');
  $('.signer-element').removeAttr('resizeable');
}
...
$(".input-field-list").on("click", ".input-field-item div", function(event){
  ...
  if (fieldLabel == "Signature") {
    ...
    $('<div class="signer-element selected-element" status="drop" resizeable="true" type="image" group="input" page="' + pageNum + '"><img src="' + baseUrl + 'assets/images/signhere.png" style="width:161px;opacity:0.5;"><br/>' + $('#inputField').text() +'</div>').appendTo(".signer-builder");
  }else{
  ...
  disableTools();
  highlightCanvas();
 if ($('#inputField').text() == recipients[recipients.length - 2].employee_name)
 {
    if (is_public)
    {
