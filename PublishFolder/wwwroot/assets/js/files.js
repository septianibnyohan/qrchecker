 ...
 */
 function loadDocuments(password){
    ...
    if (docType === "documents") {
        ...
        var posting = $.post(documentsUrl, { "folder": $("input[name=folder]").val(),  "search": $("input[name=search]").val(),  "datefilter": $("input[name=datefilter]:checked").val(), "departmentname": $("input[name=departmentname]:checked").val(), "categoryname": $("input[name=categoryname]:checked").val(), "password": password, "csrf-token": Cookies.get("CSRF-TOKEN") });
    }else{
